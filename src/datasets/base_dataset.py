...
class BaseDataset(Dataset):
    ...
    def preprocess_data(self, instance_data):
        ...
        if self.instance_transforms is not None:
            for (
                transform_name
            ) in self.instance_transforms.keys():  # for [get_spectrogram, audio, spectrogram, text, ...]
    ...
    def _filter_records_from_dataset(
        ...
        if max_text_length is not None:
            exceeds_text_length = (
                np.array([len(CTCTextEncoder.normalize_text(el["text"])) for el in index]) >= max_text_length
            )
